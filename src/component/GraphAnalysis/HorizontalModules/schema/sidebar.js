...
const Sidebar = ({ selectedItem }) => {
  ...
  const fetchRelationshipProperties = async () => {
    if (!selectedItem?.isnode && selectedItem?.group && !selectedItem.virtual) {
      try {
        const response = await axios.post(`${BASE_URL}/get_relationship_properties/`, {
        ...
        <div className="details-card">
          ...
          <div className="detail-item">
            ...
            {selectedItem.virtual ? (
              renderProperties({ count: 'Total number of relations between start and end' })
            ) : (
          ...
          {selectedItem.virtual && (
            <>
              <hr />
              <div className="detail-item">
                <span className="detail-label">path virtuel:</span>
