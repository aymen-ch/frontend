...
import VisualizationList from './HorizontalModules/containervisualization/existingvisualization';
...
const Graphe_analysis = () => {
  ...
  useEffect(() => {
    const fetchVisualizations = async () => {
      try {
        const response = await axios.get(BASE_URL + '/visualizations/');
  ...
  const handleCreateNewVisualization = () => {
    // Set an empty visualization without calling the API
  ...
  const addVisualization = (name, file, id) => {
    setVisualizations((prev) => [
      ...prev.filter(viz => viz.id !== id), // Prevent duplicates
      { id, name, file }
    ]);
  };
