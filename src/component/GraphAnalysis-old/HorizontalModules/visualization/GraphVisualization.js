...
import { FaExpand, FaCompress, FaSave, FaUndo, FaTrash, FaCog, FaSearch, FaTimes, FaSpinner, FaHome } from 'react-icons/fa';
...
const GraphVisualization = React.memo(({
  ...
  const [saveType, setSaveType] = useState('json');
  ...
  const handleSaveConfirm = async () => {
    ...
    if (saveType === 'png') {
      ...
      try {
        const response = await axios.post(BASE_URL + '/visualizations/save/', visualizationData, {
          headers: {
            'Content-Type': 'application/json',
          },
        });
        ...
        alert(t('Failed to save visualization'));
        return;
      }
    }
  ...
  return (
    <div style={containerStyle(isFullscreen)}>
      ...
      <button
        style={{ ...buttonStyle, position: 'absolute', top: '10px', left: '340px' }}
        onClick={onBackToList}
        title={t('Back to Visualizations')}
        ...
        aria-label={t('Back to Visualizations')}
      >
        <FaHome size={16} />
      </button>
        ...
        <Modal.Body>
          <Form>
            <Form.Group controlId="saveType" className="mb-3">
              ...
              >
                <option value="json">{t('Visualization (JSON)')}</option>
